...
st.markdown(
    '<p class="hero-subtitle">Track income, expenses, cards, accounts, and assets with modern analytics.</p>',
    unsafe_allow_html=True,
)
...
entry_tab, master_tab, dashboard_tab, assets_tab = st.tabs(
    ["➕ Transactions", "⚙️ Setup", "📊 Insights", "🏦 Assets"]
)
...
with entry_tab:
    ...
    with st.form("txn_form"):
        ...
        with c3:
            ...
            sub_name = (
                st.selectbox("Subcategory", options=["None"] + list(sub_map.keys()))
                if category_name
                else "None"
            )
        with c4:
        ...
        save_txn = st.form_submit_button("Save transaction", use_container_width=True)
        if save_txn and category_name and account_label:
            account_map = {
                f"{r['name']} ({r['account_type']})": int(r["id"]) for _, r in accounts.iterrows()
            }
            payload = {
        ...
        with c2:
            ...
            st.plotly_chart(
                px.bar(
                    ...
                    color="txn_type",
                    barmode="group",
                    title="Income vs Expense by Account",
                ),
                use_container_width=True,
            )

        freq = st.selectbox(
            "Period comparison", ["W", "M", "Y"],
...
with assets_tab:
    ...
    if assets.empty:
        ...
        with c2:
            ...
            st.plotly_chart(
                px.bar(by_class, x="asset_class", y="acquisition_value", title="Asset Value by Class"),
                use_container_width=True,
            )
